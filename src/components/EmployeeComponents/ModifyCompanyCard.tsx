...
function ModifyCompanyCard({ companyData, refresh }: CompanyCardProps) {
  const [openModifyModal, setOpenModifyModal] = useState<boolean>(false)
  const HandleOpenModifyModal = ()=> {
    setOpenModifyModal(prev => !prev)
  } 
  return (
    <div className="bg-white shadow-[0_4px_12px_-5px_rgba(0,0,0,0.4)] p-6 w-full max-w-sm rounded-lg overflow-hidden">
      ...
      <div className="flex flex-row items-center justify-center">
        <button onClick={HandleOpenModifyModal} className="px-3 py-1 bg-gray-200 rounded-lg hover:opacity-55">Update</button>
      </div>
      {openModifyModal && <ModifyCompanyModal refresh={refresh} companyData = {companyData} onClose={HandleOpenModifyModal} />}
    </div>
  );
}

export default ModifyCompanyCard;
